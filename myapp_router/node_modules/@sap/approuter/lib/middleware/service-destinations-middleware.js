...
async function cacheAndGetCredentials(req) {
  ...
  const hTML5RepoCredsExcluded = isHTML5RepoCredsExcluded(req);
  let appKey                   = drUtils.getApplicationKey(source);
  let serviceName              = appKey && ((appKey.bsKey && appKey.bsKey.appPrefix) || appKey.appPrefix);
  let credentials;

  // Try to get credentials from existing cache
  serviceName && attachCredentialsAndDestinations(req, serviceName, hTML5RepoCredsExcluded);
  if (req.destinationCredentials) {
    return;
  }

  // No cache, fill cache and get credentials from html5 repo
  if (!hTML5RepoCredsExcluded) {
    await html5RepoUtils.cacheHTML5Applications(req);
    serviceName = appKey && ((appKey.bsKey && appKey.bsKey.appPrefix) || appKey.appPrefix);
  ...
  // If no credentials found,fill cache and try to get credentials from destination service
...
function attachCredentialsAndDestinations(req, serviceName, hTML5RepoCredsExcluded){
  let credentials  = bsUtils.getCredentials(serviceName,true,req);
  if (credentials && (credentials.uaa || credentials.ias)) {
    req.destinationCredentials = credentials;
  }
  if (!credentials && !hTML5RepoCredsExcluded){
    credentials = html5RepoUtils.getCredentials(serviceName, req);
    const destinations = html5RepoUtils.getDestinations(req);
    if (credentials && (credentials.uaa || credentials.ias)) {
      req.destinationCredentials = credentials;
    }
    if (destinations) {
